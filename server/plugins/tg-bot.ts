...
export default defineNitroPlugin(nitro => {
  ...
  bot.on("polling_error", (err: any): void =>
  ...
  bot.on("text", async (msg: TelegramBot.Message): Promise<void> => {
    ...
    if (msg.reply_to_message?.text === TG_POST_MESSAGE) {
      ...
      const msgLoading = await botSendMessage(
        msg.chat.id,
      ...
      const resposne = await $fetch("/api/ai/send-query", {
        method: "POST",
        body: { query: requestText },
      });

      await botEditMessage(
        msg.chat.id,
        msgLoading.message_id,
        resposne.message || "Запрос не обработан"
      );
    }
  });
