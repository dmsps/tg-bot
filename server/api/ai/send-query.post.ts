...
export default defineEventHandler(async event => {
  const result = await readValidatedBody(event, aiQuerySchema.safeParse);

  if (!result.success) {
    throw createError({
      statusCode: 400,
      statusMessage: result.error.format().query?._errors.toString(),
  ...
  const openaiSendQuery = async (
    ...
    try {
      const response: OpenAI.Chat.ChatCompletion =
        await getOpenAiInstance().chat.completions.create(params);
  ...
  try {
    response = await openaiSendQuery(result.data.query) as string;
  } catch (error) {}
